...
class FullMenuViewController: UITableViewController {
    ...
    var model: [FullMenu]?   // An instance of our Holiday model needs to be wrapped into an array because we may be retrieving more than one Holiday model from the view controller passing the data
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "fullmenuid", for: indexPath)
        // Holiday selection
        let fullmenu = self.model![indexPath.row]
        
        // Based on a self-created extension of the UITableCellView to create unique text content in the cell’s textfield
        cell.makeCustomText(model: fullmenu)
